...
async function register(nameP, emailP, passwordP) {
	...
	const user = new User({
		name: nameP,
		email: emailP,
        password: await bcrypt.hash(passwordP, salt),
	});
...
async function loginByEmail(email, password) {
	...
	if (!user) {
		console.log("authentication failed - user not found, searched by email ("+email+")"); // Der eigentliche Grund wird aber auf dem Server geloggt.
		throw new Error("authentication failed"); // Dem Client wird nicht mitgeteilt, dass ein Nutzer nicht gefunden wurde und deshalb die Authentifizierung scheiterte.
	}
    ...
    if (bcrypt.compareSync(password.toString(), user.password)) {
		...
		console.error("authentication error - could not login");
