...
UserSchema.post('findOneAndDelete', findOneAndDeleteCallback);
	async function findOneAndDeleteCallback(user) {
	if (!user) return console.log('UserSchema.findOneAndDeleteCallback: no user deleted => no further action required');

	await Article.deleteByAuthorId(user._id);
}
